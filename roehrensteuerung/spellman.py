...
LOG_FOLDER = r'C:\Users\mail\Desktop\logging_folder'
def tube_log(text):
    ...
    if not os.path.isdir(LOG_FOLDER):
        ...
        if not log_name in os.listdir(LOG_FOLDER):
            with open(os.path.join(LOG_FOLDER,log_name), 'w') as fp: 
                pass
    ...
    def _send_ethernet(self):
        while True:
            ...
            try:
                ...
                tube_log('Could not send command: %s' % exc)
                self._stop()
                sys.exit()
        ...
        while True:
            ...
            if response_id == '10':
                if return_values[0] == '$':
                    ...
                    tube_log('Anode voltage setpoint out of range!')
                else:
                    raise RuntimeError
            ...
            # Parse response for changing anode current setpoint
                ...
                if return_values[0] == '$':
                    ...
                    tube_log('Anode current setpoint out of range!')
                else:
                    raise RuntimeError
            ...
            # Parse response for changing filament preheat current setpoint
                ...
                if return_values[0] == '$':
                    ...
                    tube_log('Filament preheat current out of range!')
                else:
                    raise RuntimeError
            ...
            # Parse response for changing filament current limit setpoint
                ...
                if return_values[0] == '$':
                    ...
                    tube_log('Filament current limit out of range!')
                else:
                    raise RuntimeError
            ...
            # Parse analog readback values    
                ...
                if len(return_values) == 7:
                    ...
                    tube_log('received wrong response to analog readback request.')
                    self.lv_board_temp = int(return_values[0]) * self._lv_temp_res
                    self.lv_supply_voltage = int(return_values[1]) * self._lv_supply_voltage_res
                    self.voltage = int(return_values[2]) * self._voltage_res
            ...
            # Parse response to change filament ramptime request
                ...
                if return_values[0] == '$':
                    ...
                    tube_log('Filament ramp time out of range!')
                else:
                    raise RuntimeError
            ...
            # Parse response to HV on/off request
                ...
                if return_values[0] == '$':
                    ...
                    tube_log('Wrong argument. Must be 0 or 1.')
                elif return_values[0] == '2':
                    raise ValueError('Can\'t enable HV. Interlock open.')
                    tube_log('Can\'t enable HV. Interlock open.')
                else:
                    raise RuntimeError
    ...
    def set_voltage(self, voltage):
        ...
        try:
            ...
            if dac_value not in range(4096):
                ...
                tube_log('Voltage %2.1f V out of range.' % voltage)
    ...
    def set_current(self, current):
        ...
        try:
            ...
            if dac_value not in range(4096):
                ...
                tube_log('Current %2.1f mA out of bounds.' % current)
        
        except ValueError as exc:
            print('ValueError: %s' % exc)
            tube_log('ValueError: %s' % exc)
            return
    ...
    def set_fil_preheat(self, preheat):
        ...
        try:
            ...
            if dac_value not in range(4096):
                ...
                tube_log('Current %2.1f mA out of bounds.' % preheat)
    ...
    def set_fil_maxcurrent(self, maxcurrent):
        ...
        try:
            ...
            if dac_value not in range(4096):
                ...
                tube_log('Current %2.1f mA out of bounds.' % maxcurrent)
